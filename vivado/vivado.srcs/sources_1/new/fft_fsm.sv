    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (state)
                ...
                CALC: begin
                    if (bfly_cnt < half_step - 1) begin
                        ...
                        if (group_cnt + step < N_POINTS) begin
                            ...
                            group_cnt <= 0;
                            // Zatrzymanie potoku - przejście w tryb przepłukiwania
                            valid <= 0; 
                            drain_cnt <= 0;
                            state <= FLUSH;
                        end
                ...
                FLUSH: begin
                    // Oczekiwanie aż ostatnie obliczenia danego etapu opuszczą rejestry opóźniające
                    if (drain_cnt < PIPELINE_DELAY) begin
                        ...
                        if (stage_cnt < LOG2_N) begin
                            ...
                            state <= CALC;
                            valid <= 1; // Wznowienie podawania adresów dla nowego etapu
                        end else begin
                            state <= DONE;
                        end
                    end
